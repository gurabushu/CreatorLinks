// app/artists/artist-list-client.tsx — アーティスト一覧 無限スクロール (CSR)
'use client'
...
function ArtistCard({ artist }: { artist: {
  ...
  const rating = Number(artist.averageRating)
...
export function ArtistListClient() {
  ...
  const {
    ...
    isLoading,
    isError,
  } = trpc.user.listArtists.useInfiniteQuery(
    {
  ...
  return (
    <div>
      ...
      {/* 全件表示完了 */}
      {!hasNextPage && artists.length > 0 && !isLoading && (
